...
public class UsuarioService : IBackendService<Usuario, UsuarioSyncDto> {
    ...
    // Método corrigido para corresponder à assinatura da interface IBackendService
    public async Task<Dictionary<Guid, int>> ProcessAndMapItemsAsync(IEnumerable<UsuarioSyncDto> dtos, Dictionary<string, Dictionary<Guid, int>> idMappings) {
