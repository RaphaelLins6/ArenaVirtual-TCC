// Em ArenaVirtual/Services/DatabaseService.cs
...
            // Adicionada a linha para criar a tabela de Inscricao, se ainda não existir
            await _database.CreateTableAsync<Inscricao>();
        }

        // Métodos de Inscricao (Adicionados)
        public Task<List<Inscricao>> ObterInscricoesPendentesPorCampeonatoAsync(Guid campeonatoClientAppId) =>
      _database.Table<Inscricao>()
        .Where(i => i.CampeonatoClientAppId == campeonatoClientAppId && i.Status == StatusInscricao.Pendente.ToString())
        .ToListAsync();
...
        // ⚡️ Adicionado o método ObterUsuarioPorClientAppIdAsync
...
        // ⚡️ Adicionado: Método para obter um campeonato pelo ID do capitão
...
        // ⚡️ Adicionado o método GetTimeByClientAppIdAsync
...
        // ⚡️ Adicionado: Método para obter solicitações pendentes por campeonato
        public async Task<List<Convite>> ObterSolicitacoesPendentesPorCampeonatoAsync(Guid campeonatoClientAppId) {
